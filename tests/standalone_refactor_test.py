...
class StandaloneRefactorValidator:
    ...
    def test_error_handling_file_structure(self):
        ...
        try:
            ...
            # 检查装饰器导出
            decorators = ["handle_model_error", "handle_image_error", "handle_device_error", "handle_config_error"]
    ...
    def test_model_manager_refactor(self):
        ...
        try:
            ...
            self.log_result(
                "设备管理注释", has_device_comments, "已添加职责变更说明" if has_device_comments else "缺少职责变更说明"
            )
    ...
    def test_device_manager_extensions(self):
        ...
        try:
            ...
            # 检查新增的方法
            added_methods = ["def move_model_to_device", "def move_tensor_to_device"]
    ...
    def test_image_handler_refactor(self):
        ...
        try:
            ...
            self.log_result(
                "extract_tensors 参数更新",
                bool(has_device_manager_param),
    ...
    def test_ocr_service_refactor(self):
        ...
        try:
            ...
            self.log_result(
                "错误处理导入", has_error_import, "已导入错误处理模块" if has_error_import else "缺少错误处理导入"
            )

            # 检查装饰器的使用
            decorators = ["@handle_ocr_error", "@handle_image_error"]
    ...
    def test_transformers_engine_refactor(self):
        ...
        try:
            ...
            self.log_result(
                "设备管理器使用", has_device_usage, "使用注入的设备管理器" if has_device_usage else "未使用设备管理器"
            )

            # 检查回退机制
            has_fallback = "else:" in content and "get_optimal_device()" in content
            self.log_result("回退支持", has_fallback, "包含无注入时的回退机制" if has_fallback else "缺少回退机制")
    ...
    def test_configuration_integration(self):
        ...
        try:
            ...
            self.log_result(
                "设备配置方法", has_device_config, "包含设备配置获取方法" if has_device_config else "缺少设备配置方法"
            )

            # 检查配置验证
            has_validation = "validate_config" in content or "is_valid" in content
            self.log_result("配置验证", has_validation, "包含配置验证功能" if has_validation else "缺少配置验证")
    ...
    def test_backward_compatibility(self):
        ...
        try:
            ...
            has_deprecated_funcs = (
                "def is_remote_repo" in model_path_content and "def get_loading_params" in model_path_content
            )
            self.log_result(
                "ModelPathResolver 兼容函数",
                has_deprecated_funcs,
