...
class ModelInitializer:
    ...
    def initialize_transformers_model_and_tokenizer(model_path: str, trust_remote_code: bool = True) -> Tuple[object, object]:
        ...
        try:
            ...
            is_remote_repo = (
                model_path.startswith(("http://", "https://")) or
                model_path.startswith("deepseek-ai/") or
                model_path.startswith("huggingface.co/") or
                "/" not in model_path or  # 单个名称可能是远程仓库名
                (not os.path.exists(model_path) and not os.path.exists(os.path.expanduser(model_path)))
            )
            ...
            try:
                ...
                logger.error(f"使用AutoModelForCausalLM加载失败: {str(e)}")
