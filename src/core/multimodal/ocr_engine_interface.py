...
class OCREngineInterface(ABC):
    ...
    @abstractmethod
    def process_image(self, image: Union[Image.Image, torch.Tensor], prompt: str) -> str:
        """
        处理单个图像
    ...
    @abstractmethod
    def process_batch(self, images: List[Union[Image.Image, torch.Tensor]], prompts: List[str]) -> List[str]:
        """
        批量处理图像
