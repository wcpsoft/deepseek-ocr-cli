...
from src.core.utils.exception_handler import ExceptionHandler, OCRException, SafeExecution
...
class OCRResultProcessor:
    ...
    def save_results(self, filename: str = "result.mmd") -> Optional[str]:
        """
        保存OCR结果到文件
    ...
    def save_error_report(self, filename: str = "error_report.txt") -> Optional[str]:
        """
        保存错误报告到文件
    ...
    def process_images(self, images: List[Any], ocr_engine) -> bool:
        ...
        try:
            for i, image in enumerate(images):
                ...
                # 使用安全执行器处理单个图像
                def process_single():
                    return self._process_single_image(image, i, ocr_engine)
                
                result = SafeExecution.safe_execute(
                    process_single, 
            ...
            logger.error(f"批量处理过程中发生OCR异常: {str(e)}")
            if e.details:
                logger.error(f"异常详情: {e.details}")
            return False
        except Exception as e:
            logger.error(f"批量处理过程中发生严重错误: {str(e)}")
            return False
    
    def _process_single_image(self, image: Any, image_index: int, ocr_engine) -> Optional[str]:
        """
        处理单个图像
