...
该模块保留了原始DeepSeek-OCR的图像处理算法，并针对Apple Silicon(MPS)设备添加了特殊优化。
"""
        ...
        try:
            ...
            if result and len(result) > 0 and result[0]:
                ...
                for i, item in enumerate(result[0]):
                    ...
                    if item is not None:
                        logger.debug(f"result[0][{i}]值的类型: {type(item)}")
...
try:
    ...
    logger.debug("成功注册DeepseekVLV2Processor")
except Exception as e:
    logger.warning(f"注册处理器失败: {str(e)}")