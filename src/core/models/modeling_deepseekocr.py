...
from src.core.models.conversation import get_conv_template
            ...
            for image, crop_shape in zip(images, images_spatial_crop):
                ...
                if images_in_this_batch:
                    ...
                    inputs_embeds[idx].masked_scatter_(
                        images_seq_mask[idx].unsqueeze(-1).to(device), images_in_this_batch.to(device)
                    )

                idx += 1

        # 确保position_ids不为None
        ...
        if past_key_values is not None:
            if isinstance(past_key_values, Cache):
                ...
                try:
                    ...
                    max_cache_length = max_cache_shape[0] if max_cache_shape is not None else None
                except AttributeError:
                    # 如果get_max_cache_shape方法不存在，回退到get_max_cache
                    try:
                        max_cache_length = past_key_values.get_max_cache()
                    except AttributeError:
                        # 如果get_max_cache也不存在，回退到get_max_length
