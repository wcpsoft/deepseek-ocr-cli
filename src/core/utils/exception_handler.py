...
class MemoryException(OCRException):
    ...
    内存不足异常
    """
    def __init__(self, message: str = "内存不足", details: Optional[Dict[str, Any]] = None):
...
class ModelLoadException(OCRException):
    ...
    模型加载异常
    """
    def __init__(self, message: str = "模型加载失败", details: Optional[Dict[str, Any]] = None):
...
class ImageProcessException(OCRException):
    ...
    图像处理异常
    """
    def __init__(self, message: str = "图像处理失败", details: Optional[Dict[str, Any]] = None):
...
class SafeExecution:
    ...
    def safe_execute(func: Callable, *args, stop_on_error: bool = True, **kwargs) -> Any:
        ...
        try:
            ...
            if stop_on_error:
                logger.error("停止处理")
                raise OCRException(f"处理失败: {str(e)}")
            else:
                logger.warning("继续处理下一项")
                return None