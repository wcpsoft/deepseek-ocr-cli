...
def is_mps_device() -> bool:
    ...
    try:
        ...
        logger.warning(f"检测MPS设备时发生错误: {str(e)}")
        return False
...
def optimize_tensor_for_mps(tensor: torch.Tensor, 
                           max_sequence_length: int = 512,
                           max_image_blocks: int = 10) -> torch.Tensor:
    """
    优化张量以适应MPS设备的限制
...
def mps_safe_tensor_creation(shape: Tuple[int, ...], 
    ...
    try:
        ...
        logger.error(f"在设备{device}上创建张量失败: {str(e)}")
        # 回退到CPU
        logger.info("回退到CPU设备创建张量")
        return torch.zeros(shape, dtype=dtype, device='cpu', **kwargs)


def mps_safe_model_load(model: torch.nn.Module, 
                       checkpoint_path: str,
                       map_location: Optional[Union[str, torch.device]] = None) -> torch.nn.Module:
    """
    在MPS设备上安全加载模型
    ...
    try:
        ...
        logger.error(f"在{map_location}设备上加载模型失败: {str(e)}")
...
def get_mps_memory_info() -> dict:
    ...
    try:
        ...
        logger.error(f"获取MPS内存信息失败: {str(e)}")
        return {"error": str(e)}