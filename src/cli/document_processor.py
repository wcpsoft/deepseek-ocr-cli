...
try:
    ...
    try:
        ...
        raise ImportError("请安装PyMuPDF包: pip install PyMuPDF") from e
...
class DocumentProcessor:
    ...
    def _convert_to_pdf(self, input_path: Path, output_dir: Path) -> Path:
        ...
        try:
            ...
            raise RuntimeError("LibreOffice转换超时") from e
        except Exception as e:
            raise RuntimeError(f"转换过程中发生错误: {e!s}") from e
    ...
    def _perform_ocr(self, images: list[Image.Image], output_dir: Path):
        ...
        # 为当前任务创建独立的输出子目录，与Web API保持一致
        import uuid

        from src.core.api.utils.helpers import create_job_directories

        job_id = uuid.uuid4().hex[:8]
        _, task_output_dir = create_job_directories(output_dir, job_id, is_web_api=False)
        ...
        # 执行OCR处理
        ocr_engine.process(images, str(task_output_dir))
