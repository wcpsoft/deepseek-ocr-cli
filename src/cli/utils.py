...
def check_model_availability(project_root: Path) -> tuple[bool, str | None]:
    """
    检查模型是否可用
...
def get_appropriate_dtype_compat(device: torch.device) -> torch.dtype:
    """
    根据设备类型获取适当的数据类型（保持向后兼容）
    ...
    Returns:
        torch.dtype: 适当的数据类型
    """
    return get_appropriate_dtype(device)


def should_use_bfloat16_compat(device: torch.device) -> bool:
    """
    判断是否应该使用bfloat16数据类型（保持向后兼容）
    ...
    Returns:
        bool: 是否应该使用bfloat16
    """
    return should_use_bfloat16(device)
